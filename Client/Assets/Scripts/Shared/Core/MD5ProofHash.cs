...
{
    public sealed class MD5ProofHash : IProofHash
    {
        /*
         * Round shift values
         */
        ...
        private static byte[] Calculate(byte[] input)
        {
            ...
            byte[] result = new byte[sizeof(uint) * 4];
            BitConverter.GetBytes(a0).CopyTo(result, 0);
            BitConverter.GetBytes(b0).CopyTo(result, 1 * sizeof(uint));
            BitConverter.GetBytes(c0).CopyTo(result, 2 * sizeof(uint));
            BitConverter.GetBytes(d0).CopyTo(result, 3 * sizeof(uint));
        ...
        {
            byte[] result = Calculate(Encoding.Unicode.GetBytes(value));
